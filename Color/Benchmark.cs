...
{
    [Benchmark]
    public void MainFceBenchmark() => MainFce(out _, out _, out _);

    [Benchmark]
    public void MainFceJavaBenchmark() => MainFceJava(out _);

    [Benchmark]
    public void MainNewBenchmark() => MainNew(out _, out _, out _, out _);

    [Benchmark]
    public void MainForm() => MainForm(out _);

    public void MainFce(out vec3 color0, out vec3 color1, out vec3 color2)
    {
    ...
    public void MainFceJava(out vec3 color0)
    {
    ...
    public void MainNew(out vec3 color0, out vec3 color1, out vec3 color2, out vec3 color3)
    {
        vec3 cent1 = new vec3(0.7f, 0.2f, 0.5f);
        vec3 cent2 = new vec3(1f, 0.5f, 0.7f);
        vec3 cent3 = new vec3(0.5f, 0.7f, 0.2f); ;
    ...
    public void MainForm(out vec4 FragColor)
    {
        //Here we sample the texture based on the Uv coordinates of the fragment
        //vec4 a = new vec4(240, 127, 180,255);
        vec4 a = new vec4(20, 150, 140,255);
        vec4 b = new vec4(255, 255, 255, 255);

        vec3 hsb = RgbToHsb(a.xyz);
        vec3 keyColorPoint = new vec3(220, 0, 0);
        vec2 keyDistance = Vzdalenost(hsb, keyColorPoint);

        if (keyDistance.x < 100 && hsb.y > 0.35 && hsb.z > 0.35)
        {
            FragColor = b;
        }
        else
